...
def prep_xy(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series, int]:
    """
...
def train_xgb(X, y, num_cls, seed=42):
    """
    Train an ``XGBClassifier`` and evaluate log‑loss.
    ...
    tuple[float, XGBClassifier]
        Tuple of (log_loss, trained model).
    """
    ...
    return log_loss(y_test, y_hat), clf
...
def main(game_id: str = "0022400001"):
    """Train baseline and sequence models for ``game_id`` and persist them."""

    # -------- baseline (memory‑0) ------------------------------------------ #
    base_df = load_csv("baseline", game_id)
    Xb, yb, k_base = prep_xy(base_df)
    ll_base, base_model = train_xgb(Xb, yb, k_base)

    # -------- sequence (memory‑3) ------------------------------------------ #
    seq_df = load_csv("sequence", game_id)
    Xs, ys, k_seq = prep_xy(seq_df)
    ll_seq, seq_model = train_xgb(Xs, ys, k_seq)

    # -------- persist models ----------------------------------------------- #
    os.makedirs("models", exist_ok=True)
    joblib.dump(base_model, os.path.join("models", "baseline.pkl"))
    joblib.dump(seq_model, os.path.join("models", "sequence.pkl"))
    base_model.save_model(os.path.join("models", "baseline_xgb.json"))
    seq_model.save_model(os.path.join("models", "sequence_xgb.json"))
